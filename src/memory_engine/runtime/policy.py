        ...
        for extractor_name in api_entry.extractors:
            ...
            candidates.append(
                CandidateMemory(
                    ...
                    sensitivity=max(
                        [api_entry.sensitivity_hint, extracted["sensitivity"]],
...
def evaluate_event(
    ...
    candidates = _extract_candidates(event, api_entry, memory_entries)
    if not candidates:
        return DecisionEnvelope(
            config_snapshot_id=snapshot.id if snapshot else None,
            event_id=new_id("evt"),
    ...
    if any(SENSITIVITY_RANK[candidate.sensitivity] >= SENSITIVITY_RANK["S3_CONFIDENTIAL"] for candidate in candidates):
        return DecisionEnvelope(
            config_snapshot_id=snapshot.id if snapshot else None,
            event_id=new_id("evt"),
            action="BLOCK",
