...
const publicationSeed = (): PublicationSnapshot[] => [
  {
    ...
    snapshotHash: "mock-hash-api-001",
    scope: "global",
    tenant: "all",
    environment: "staging-local",
    publishedAt: now(),
    publishedBy: "operator@memoryengine.local",
    releaseNotes: "Initial active snapshot for the operator console.",
    apiOntology: { id: "cfg_api_1", name: "Core API Ontology", version: 3, kind: "api-ontology", status: "published" },
    memoryOntology: { id: "cfg_memory_1", name: "Memory Ontology", version: 5, kind: "memory-ontology", status: "validated" },
    policyProfile: { id: "cfg_policy_1", name: "Default Policy Profile", version: 7, kind: "policy-profile", status: "approved" },
  },
  {
    id: "snap_102",
    ...
    snapshotHash: "mock-hash-policy-002",
    scope: "global",
    tenant: "all",
    environment: "staging-local",
    publishedAt: now(),
    publishedBy: "approver@memoryengine.local",
    releaseNotes: "Current policy profile used for local evaluation.",
    apiOntology: { id: "cfg_api_1", name: "Core API Ontology", version: 3, kind: "api-ontology", status: "published" },
    memoryOntology: { id: "cfg_memory_1", name: "Memory Ontology", version: 5, kind: "memory-ontology", status: "validated" },
    policyProfile: { id: "cfg_policy_1", name: "Default Policy Profile", version: 7, kind: "policy-profile", status: "published" },
  },
];
...
export async function mockPublishConfig(configId: string): Promise<PublicationSnapshot> {
  ...
  const snapshot: PublicationSnapshot = {
    ...
    snapshotHash: `mock-hash-${config.id}`,
    scope: config.scope,
    tenant: config.tenant,
    environment: config.environment,
    publishedAt: now(),
    publishedBy: state.session?.user.email ?? "operator@memoryengine.local",
    releaseNotes: "Published from the operator console.",
    apiOntology: { id: "cfg_api_1", name: "Core API Ontology", version: 3, kind: "api-ontology", status: "published" },
    memoryOntology: { id: "cfg_memory_1", name: "Memory Ontology", version: 5, kind: "memory-ontology", status: "validated" },
    policyProfile: { id: "cfg_policy_1", name: "Default Policy Profile", version: 7, kind: "policy-profile", status: "approved" },
  };
